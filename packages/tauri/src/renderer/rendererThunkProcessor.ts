...
/**
 * Handles thunk execution in the renderer (Tauri webview) process.
 *
 * The Tauri analogue of Electron's WebContents identity is a webview label
 * (string), so the processor tracks `currentWindowLabel` rather than a numeric
 * window id. Action and thunk lifecycle traffic is sent over Tauri commands by
 * the supplied `actionSender` / `thunkRegistrar` / `thunkCompleter` callbacks.
 */
export class RendererThunkProcessor extends BaseThunkProcessor {
  /** Webview label this processor is running inside */
  private currentWindowLabel?: string;

  // Function to send actions to the Tauri backend
  ...
  // Function to register thunks with the Tauri backend
  ...
  // Function to notify thunk completion to the backend
  private thunkCompleter?: (thunkId: string, error?: string) => Promise<void>;
  ...
  public initialize(options: {
    ...
    thunkCompleter: (thunkId: string, error?: string) => Promise<void>;
    actionCompletionHandler?: (actionId: string, callback: (result: unknown) => void) => () => void;
    actionCompletionTimeoutMs?: number;
  }): void {
    debug('tauri', '[RENDERER_THUNK] Initializing with options:', options);
    this.currentWindowLabel = options.windowLabel;
    ...
    if (options.actionCompletionTimeoutMs !== undefined) {
      ...
      debug('tauri', '[RENDERER_THUNK] Updated timeout:', this.actionCompletionTimeoutMs);
    }

    debug('tauri', `[RENDERER_THUNK] Initialized for webview label "${options.windowLabel}"`);
  }
  ...
  public async executeThunk<S extends AnyState>(
    ...
    // Create a Thunk instance. Tauri identifies webviews by string labels
    // rather than numeric ids; the wire protocol uses sourceLabel separately.
    // We pass 0 as a sentinel for the legacy sourceWindowId field.
  ...
  /**
   * Dispatch an action directly through the configured `actionSender`. Used
   * for non-thunk scenarios where a thunk-context dispatch is not available.
   */
  public async dispatchAction(
    ...
    debug('tauri', '[RENDERER_THUNK] dispatchAction called with:', { action, payload, parentId });
  ...
  public destroy(): void {
    ...
    this.currentWindowLabel = undefined;
