...
// Populated by the application via `registerActionMapping`.
const actionToStateKeyMap = new Map<string, string[]>();

type StateProvider = () => Promise<Record<string, unknown> | null>;
let stateProvider: StateProvider | null = null;

/**
 * Configures how the validator reads current state. Wired by the bridge client
 * to the renderer's local store replica or to a `get_state` Tauri call.
 */
export function setActionValidatorStateProvider(provider: StateProvider | null): void {
  stateProvider = provider;
}

async function readState(): Promise<Record<string, unknown> | null> {
  return stateProvider ? stateProvider() : null;
}
...
export async function canDispatchAction(action: Action): Promise<boolean> {
  ...
  // Get current state via the configured provider
