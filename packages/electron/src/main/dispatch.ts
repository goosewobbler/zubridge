...
import { getCurrentWebContentsId } from '../utils/windows.js';
...
export function createDispatch<S extends AnyState>(
  ...
  const internalDispatch = (
    ...
    return getCurrentWebContentsId().then(async (sourceWindowId) => {
      try {
        console.log(`[DISPATCH] Using source window ID: ${sourceWindowId}`);
        ...
        if (typeof actionOrThunk === 'function') {
          ...
          return new Promise(async (resolve, reject) => {
            try {
              ...
              // Execute the thunk with the source window ID
              try {
                // Initialize the processor with the current window ID for this dispatch
                mainThunkProcessor.initialize({
                  stateManager,
                  sourceWindowId,
                });
          ...
          try {
            ...
            // Add the source window ID to the action
            (actionObj as any).__sourceWindowId = sourceWindowId;
