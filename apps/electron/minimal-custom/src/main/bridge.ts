import { createCoreBridge, createDispatch, type ZustandBridge } from '@zubridge/electron/main';
 ...
 */
export function createBridge(store: StateManager<AnyState>): ZustandBridge {
  console.log('[Custom Mode] Creating bridge with custom state manager');

  // Create the core bridge with our custom store
  const coreBridge = createCoreBridge(store);
